...
namespace TUnit.Mixed_1._68._0;
...
public sealed class CastDisposalShowcase
{
    private CancellationTokenSource _viaCast = null!;
    private CancellationTokenSource _viaAs = null!;
    private object _viaObject = null!;

    [Before(Test)]
    public void CreateSources()
    {
        _viaCast = new CancellationTokenSource();
        _viaAs = new CancellationTokenSource();
        _viaObject = new CancellationTokenSource();
    }

    [Test]
    public async Task Sources_are_live_before_cleanup()
    {
        await Assert.That(_viaCast.IsCancellationRequested).IsFalse();
        await Assert.That(_viaAs.IsCancellationRequested).IsFalse();
        await Assert.That(_viaObject).IsTypeOf<CancellationTokenSource>();
    }

    // Deliberately ugly: every member is disposed through a cast. 1.68.0 stops TUnit0023
    // from reporting these as never disposed; 1.67.0 flagged them.
    [After(Test)]
    public void DisposeThroughCasts()
    {
        ((IDisposable)_viaCast).Dispose();
        (_viaAs as IDisposable)?.Dispose();
        ((IDisposable)_viaObject).Dispose();
    }
}
