...
[docs](https://tunit.dev/docs/intro), pinned to TUnit 1.67.0. Every test asserts the behaviour
...
Version-specific notes, verified against 1.67.0 rather than the docs:
...
- `ProceedOnFailureShowcase` and `Inconclusive_at_run_time` are `[Explicit]` because they need a failing / inconclusive result; run them with a `--treenode-filter`. `InconclusiveTestException` is reported as *failed* on 1.67.0.
...
- An assembly-level `[ParallelLimiter<T>]` replaces class- and method-level ones on 1.67.0 (docs: Method > Class > Assembly); the assembly-level form therefore lives in `TUnit.Patterns.Policies 1.67.0` together with the other assembly-wide attributes.
...
```bash
dotnet run --project "TUnit.Patterns 1.67.0/TUnit.Patterns 1.67.0.csproj"
```
