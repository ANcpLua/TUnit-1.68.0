...
public sealed class ContextTests
{
    ...
    // 1.66.16 (breaking): GetIsolatedName is lowercase, "test_{id}_{name}" instead of "Test_{id}_{name}",
    // consistent with GetIsolatedPrefix. Resource names persisted by an older run no longer match.
    [Test]
