...
- `TUnit.Mocks 1.64.0/MockEssentials.cs` — first mock, loose-mode smart defaults, inline lambda
  matchers, argument capture, sequential `.Then()` setups, strict mode.
- `TUnit.Mocks 1.64.0/StatefulConnectionTests.cs` — state-machine mocking (`InState` /
  `TransitionsTo`), typed event raising, subscription tracking.
- `TUnit.Mocks 1.64.0/PendingTaskTests.cs` — **new in 1.62.0**: `Returns(async () => …)` on async
...
- `TUnit.Mocks 1.64.0/RuntimeAutoStubTests.cs` — **new in 1.63.0**: loose mocks emit functional
...
- `TUnit.Mocks 1.64.0/InternalsAccessTests.cs` — **experimental in 1.63.0**: name, mock, and
...
```bash
dotnet run --project "TUnit.Mocks 1.64.0/TUnit.Mocks 1.64.0/TUnit.Mocks 1.64.0.csproj"
```
