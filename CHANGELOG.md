...
| `<TUnitMocksExperimentalInternalsAccess>` + `<TUnitMocksInternalsAccess Include>` | MSBuild | config | `TUnit.Mocks 1.68.0.csproj`, `InternalsAccessTests.cs` | `InternalsAccessTests` | 1.63.0 (experimental) | still experimental in 1.68.0; IDEs report false `CS0122` (see divergences) |
...
- `<TUnitMocksInternalsAccess>` publicizes the compiler's reference list only: `TUnitMocksPublicizeReferences` swaps the rewritten copy into `ReferencePathWithRefAssemblies` (and into `/reference:` on the csc command line), while `ReferencePath` keeps the original assembly. IDE tooling that builds its Roslyn workspace from `ReferencePath` therefore underlines `IQuotaPolicy` in `InternalsAccessTests.cs` with `CS0122` / `CS1503` while `dotnet build` is clean — the build is the authority (reported upstream as #6836). Probed with `dotnet msbuild "TUnit.Mocks 1.68.0/…csproj" -t:Compile -p:SkipCompilerExecution=true -p:ProvideCommandLineArgs=true -getItem:ReferencePath -getItem:ReferencePathWithRefAssemblies -getItem:CscCommandLineArgs`.
